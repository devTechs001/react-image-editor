...
export const uploadImage = async (req, res) => {
  try {
    ...
    // Process image
    const processedImage = await sharp(req.file.buffer).resize(2048, 2048, {
      fit: 'inside',
...
export const resizeImage = async (req, res) => {
  try {
    ...
    const response = await fetch(image.url);
    const buffer = await response.arrayBuffer();
    const resizedImage = await sharp(Buffer.from(buffer)).resize(width, height, {
      fit
    }).toBuffer();
...
export const cropImage = async (req, res) => {
  try {
    ...
    const response = await fetch(image.url);
    const buffer = await response.arrayBuffer();
    const croppedImage = await sharp(Buffer.from(buffer)).extract({
      left,
      top,
      width,
      height
