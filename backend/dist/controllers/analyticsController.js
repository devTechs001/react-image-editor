...
exports.getDashboard = async (req, res, next) => {
  try {
    ...
    // Get counts
    const [projectCount, assetCount, exportCount, recentProjects, aiUsage] = await Promise.all([Project.countDocuments({
      user: userId,
      status: {
        $ne: 'deleted'
      }
    ...
    }).sort('-updatedAt').limit(5).select('name type thumbnail updatedAt'), AILog.aggregate([{
      $match: {
        user: userId,
        createdAt: {
          $gte: thirtyDaysAgo
...
exports.getUsageStats = async (req, res, next) => {
  try {
    ...
    const [projects, exports, aiLogs] = await Promise.all([Project.aggregate([{
      $match: {
        user: userId,
        createdAt: {
          $gte: startDate,
...
exports.getExportStats = async (req, res, next) => {
  try {
    ...
    const recentExports = await Export.find({
      user: userId
    }).sort('-createdAt').limit(10).populate('project', 'name');
    res.json({
      success: true,
      data: {
        byFormat: stats,
...
exports.getAIStats = async (req, res, next) => {
  try {
    const stats = await AILog.getUserStats(req.user._id, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), new Date());
    res.json({
      success: true,
      data: {
        stats,
...
exports.getAdminOverview = async (req, res, next) => {
  try {
    ...
    const [totalUsers, newUsersToday, newUsersThisMonth, totalProjects, totalExports, totalAIOperations] = await Promise.all([User.countDocuments(), User.countDocuments({
      createdAt: {
        $gte: today
      }
    }), User.countDocuments({
    ...
    }), Project.countDocuments(), Export.countDocuments({
      status: 'completed'
...
exports.getUserStats = async (req, res, next) => {
  try {
    ...
    const users = await User.find().sort(sort).skip((page - 1) * limit).limit(parseInt(limit)).select('-password -refreshTokens');
