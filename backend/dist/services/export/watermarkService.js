...
async function addWatermark(imageBuffer, watermarkConfig) {
  ...
  try {
    ...
    if (text) {
      ...
      // Apply opacity
      watermarkImage = watermarkImage.joinChannel(Buffer.from(Array(Math.round(watermarkWidth * (await watermarkImage.metadata()).height * (opacity * 255))).fill(255)));
...
async function removeWatermark(imageBuffer, watermarkArea) {
  ...
  const {
    inpaint
  } = require('../ai/inpainting');
  try {
    return await inpaint(imageBuffer, {
      x,
      y,
      width,
      height
