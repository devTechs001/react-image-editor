...
const crop = async (imageBuffer, options = {}) => {
  try {
    const {
      aspectRatio = 1,
      focusOn = 'auto',
      // 'auto', 'face', 'center', 'rule-of-thirds'
      padding = 0.1
    } = options;
    ...
    if (focusOn === 'auto' || focusOn === 'object') {
      ...
      const image = await sharp(imageBuffer).resize(640, 640, {
        fit: 'inside'
      }).raw().toBuffer({
        resolveWithObject: true
      });
      const tensor = tf.tensor3d(new Uint8Array(image.data), [image.info.height, image.info.width, image.info.channels]);
      const predictions = await model.detect(tensor);
      tensor.dispose();
      if (predictions.length > 0) {
        // Find the most prominent object
        const mainObject = predictions.reduce((prev, current) => prev.score > current.score ? prev : current);
