...
async function inpaintWithStability(imageBuffer, mask, options) {
  ...
  const response = await axios.post('https://api.stability.ai/v1/generation/stable-diffusion-xl-1024-v1-0/inpainting', formData, {
    headers: {
      ...formData.getHeaders(),
...
async function simpleInpaint(imageBuffer, mask) {
  ...
  const result = await sharp(imageBuffer).blur(2).toBuffer();
  return result;
}
...
async function outpaint(imageBuffer, options = {}) {
  ...
  return await inpaint(extendedBuffer, maskBuffer, {
    prompt
  });
}
