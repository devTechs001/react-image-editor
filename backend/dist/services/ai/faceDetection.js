...
async function detectWithAzure(imageBuffer, options) {
  const response = await axios.post(`${config.azure.faceEndpoint}/face/v1.0/detect`, imageBuffer, {
    headers: {
    ...
    params: {
      ...
      returnFaceAttributes: options.returnAttributes ? 'age,gender,emotion,facialHair,glasses,headPose,smile,mask,blur,exposure,noise,occlusion,accessories,hair,makeup,accessories' : 'none'
    }
  });
...
async function analyzeFaceAttributes(imageBuffer, faceId) {
  ...
  return {
    face,
    analysis: {
      dominantEmotion: face.attributes?.emotions?.reduce((a, b) => a.confidence > b.confidence ? a : b)?.type,
      estimatedAge: face.attributes?.ageRange ? (face.attributes.ageRange.low + face.attributes.ageRange.high) / 2 : null,
