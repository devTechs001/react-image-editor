...
async function analyzeWithAWS(text) {
  ...
  return {
    score: sentimentResult.SentimentScore ? sentiment.SentimentScore.Positive - sentimentResult.SentimentScore.Negative : 0,
...
async function extractKeywords(text, options = {}) {
  ...
  try {
    if (config.google.applicationCredentials) {
      ...
      return result.entities?.filter(e => e.type === 'CONSUMER_GOOD' || e.type === 'OTHER').slice(0, maxKeywords).map(e => ({
        text: e.name,
        salience: e.salience,
        type: e.type
      })) || [];
    ...
    return Object.entries(frequency).sort((a, b) => b[1] - a[1]).slice(0, maxKeywords).map(([word, count]) => ({
      text: word,
      count
    }));
  } catch (error) {
