...
aiMemorySchema.statics.getUserMemory = async function (userId, options = {}) {
  ...
  return this.find(query).sort(sort).limit(limit).populate('relatedMemories');
};
...
aiMemorySchema.statics.getPersonalizedTips = async function (userId, context = {}) {
  ...
  return this.find(query).sort({
    'learningData.personalizedTips.effectiveness': -1
  }).limit(limit).select('learningData.personalizedTips');
};
