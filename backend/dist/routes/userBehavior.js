...
router.post('/analyze', auth, validate([body('sessionId').notEmpty().withMessage('Session ID is required'), body('userId').notEmpty().withMessage('User ID is required'), body('behaviors').isArray().withMessage('Behaviors must be an array'), body('analysis').optional().isObject()]), async (req, res) => {
  try {
    const {
      sessionId,
      userId,
...
router.post('/recommendations', auth, validate([body('userId').notEmpty().withMessage('User ID is required'), body('context').optional().isObject(), body('preferences').optional().isObject(), body('recentBehaviors').optional().isArray()]), async (req, res) => {
  try {
    const {
      userId,
      context = {},
    ...
    const userHistory = await UserBehavior.find({
      userId
    }).sort({
      timestamp: -1
    }).limit(100);
    ...
    const aiMemory = (await AIMemory.findOne({
      userId
    })) || {};
...
router.post('/adaptive-feedback', auth, validate([body('userId').notEmpty().withMessage('User ID is required'), body('operation').notEmpty().withMessage('Operation is required'), body('result').optional().isObject(), body('userHistory').optional().isArray(), body('preferences').optional().isObject()]), async (req, res) => {
  try {
    const {
      userId,
      operation,
...
// Update user preferences
router.put('/preferences', auth, validate([body('userId').notEmpty().withMessage('User ID is required'), body('preferences').isObject().withMessage('Preferences must be an object')]), async (req, res) => {
  try {
    const {
      userId,
      preferences
...
async function generateRecommendations(userId, data) {
  ...
  const recentTools = recentBehaviors.filter(b => b.type === 'tool_selection').map(b => b.data.tool);
...
async function generateBehaviorAnalytics(behaviorData) {
  ...
  behaviorData.forEach(session => {
    ...
    if (session.analysis?.workingHours) {
      session.analysis.workingHours.forEach(({
        hour,
        activity
      }) => {
