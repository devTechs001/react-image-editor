...
const { exportImage } = require('../services/export/imageExport');
const { exportVideo } = require('../services/export/videoExport');
const { exportGif } = require('../services/export/gifExport');
const { exportPdf } = require('../services/export/pdfExport');
...
exports.createExport = async (req, res, next) => {
  try {
    ...
    switch (format.toLowerCase()) {
      ...
      case 'webp':
      case 'tiff':
        result = await exportImage(project, { format, ...options });
        break;
      case 'gif':
      ...
      case 'mp4':
      case 'webm':
        result = await exportVideo(project, { format, ...options });
        break;
      case 'pdf':
