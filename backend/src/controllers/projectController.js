...
exports.createProject = async (req, res, next) => {
  try {
    const { name, type, canvas, description, template } = req.body;
...
exports.updateProject = async (req, res, next) => {
  try {
    const allowedFields = [
      'name', 'description', 'canvas', 'layers', 'adjustments',
      'filters', 'settings', 'tags', 'thumbnail'
    ];
...
exports.duplicateProject = async (req, res, next) => {
  try {
    ...
    original.sharing = { isPublic: false };
...
exports.shareProject = async (req, res, next) => {
  try {
    ...
    if (isPublic) {
      ...
      project.sharing = { isPublic: false };
    }
    ...
    res.json({
      success: true,
      data: {
        shareLink: project.sharing.shareLink
          ? `${process.env.FRONTEND_URL}/shared/${project.sharing.shareLink}`
          : null,
...
exports.updateLayers = async (req, res, next) => {
  try {
    ...
    project.addToHistory('layers_update', { layerCount: layers.length });
    await project.save();
...
exports.updateAdjustments = async (req, res, next) => {
  try {
    ...
    project.adjustments = { ...project.adjustments, ...adjustments };
    project.addToHistory('adjustments_update', adjustments);
    await project.save();

    res.json({
      success: true,
      data: { adjustments: project.adjustments }
    });
  } catch (error) {
    next(error);
  }
