...
const { uploadToStorage, deleteFromStorage } = require('../services/storage/s3Storage');
...
exports.uploadAsset = [
  ...
  async (req, res, next) => {
    try {
      ...
      const { folder = 'root', projectId } = req.body;
...
exports.getAssets = async (req, res, next) => {
  try {
    const { type, folder, search, page = 1, limit = 50 } = req.query;
