...
const { uploadToStorage } = require('../services/storage/s3Storage');
...
exports.enhanceImage = async (req, res, next) => {
  try {
    ...
    const { enhanceType = 'general' } = req.body;
    const startTime = Date.now();

    const result = await aiService.imageEnhancement.enhance(
      req.file.buffer,
      enhanceType
    );
...
exports.upscaleImage = async (req, res, next) => {
  try {
    ...
    const result = await aiService.imageUpscaling.upscale(
      req.file.buffer,
      parseInt(scale)
    );
...
exports.styleTransfer = async (req, res, next) => {
  try {
    ...
    const result = await aiService.styleTransfer.transfer(
      content[0].buffer,
      style[0].buffer
    );
    ...
    const outputUrl = await uploadToStorage(
      result.buffer,
      `ai/style-transfer/${Date.now()}.png`,
      'image/png'
    );
...
exports.removeObject = async (req, res, next) => {
  try {
    ...
    const { mask } = req.body; // Base64 mask
    const startTime = Date.now();

    const result = await aiService.inpainting.removeObject(
      req.file.buffer,
      Buffer.from(mask, 'base64')
    );

    const outputUrl = await uploadToStorage(
      result.buffer,
      `ai/object-removed/${Date.now()}.png`,
      'image/png'
    );
...
exports.generateImage = async (req, res, next) => {
  try {
    ...
    await AILog.create({
      ...
      settings: { width, height, steps, guidance, seed },
      metrics: { processingTime: Date.now() - startTime },
      creditsUsed: 3
    });
...
exports.inpaint = async (req, res, next) => {
  try {
    ...
    const result = await aiService.inpainting.inpaint(
      image[0].buffer,
      mask[0].buffer,
      prompt
    );
...
exports.smartCrop = async (req, res, next) => {
  try {
    ...
    const { aspectRatio, focusOn } = req.body;
    const startTime = Date.now();
