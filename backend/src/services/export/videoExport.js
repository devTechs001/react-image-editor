...
async function exportVideo(project, options = {}) {
  ...
  try {
    ...
    for (let i = 0; i < totalFrames; i++) {
      ...
      await renderFrameAtTime(project, time, framePath, { width, height });
    ...
    await runFfmpegVideo(frames, outputPath, { fps, quality, bitrate, format });
...
function runFfmpegVideo(frames, outputPath, options) {
  return new Promise((resolve, reject) => {
    const { fps, quality, bitrate, format } = options;
