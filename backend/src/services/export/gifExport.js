...
async function exportGif(project, options = {}) {
  ...
  try {
    ...
    await runFfmpeg(frames, outputPath, { fps, quality, colors, dither });
...
function runFfmpeg(frames, outputPath, options) {
  return new Promise((resolve, reject) => {
    const { fps, quality, colors, dither } = options;
    ...
    const args = [
      ...
      '-i', framePattern,
      '-vf', `split[s0][s1];[s0]palettegen=max_colors=${colors}[p];[s1][p]paletteuse=dither=${dither ? 'bayer:bayer_scale=5' : 'none'}`,
      '-y',
      outputPath
    ];
