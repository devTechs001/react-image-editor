...
// Resize image
const resize = async ({ buffer, width, height, fit = 'cover', format = 'png' }) => {
  try {
...
// Compress image
const compress = async ({ buffer, quality = 80, format }) => {
  try {
...
// Convert format
const convert = async ({ buffer, format, options = {} }) => {
  try {
    const result = await sharp(buffer)
...
// Apply adjustments
const applyAdjustments = async ({ buffer, adjustments }) => {
  try {
    let pipeline = sharp(buffer);
...
// Crop image
const crop = async ({ buffer, x, y, width, height }) => {
  try {
    const result = await sharp(buffer)
...
// Rotate image
const rotate = async ({ buffer, angle, background = '#ffffff' }) => {
  try {
    const result = await sharp(buffer)
...
// Flip image
const flip = async ({ buffer, direction = 'horizontal' }) => {
  try {
    let pipeline = sharp(buffer);
...
const exportProject = async (project, settings, onProgress) => {
  try {
    const { format = 'png', quality = 100, scale = 1 } = settings;
    ...
    for (const layer of project.layers.filter(l => l.visible)) {
      if (layer.data?.imageUrl) {
        ...
        const response = await axios.get(layer.data.imageUrl, { responseType: 'arraybuffer' });
        
        let layerBuffer = Buffer.from(response.data);
        
        // Apply layer transformations
        layerBuffer = await sharp(layerBuffer)
          .resize(Math.round(layer.size.width * scale), Math.round(layer.size.height * scale))
