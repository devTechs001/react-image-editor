...
async function detectObjects(imageBuffer, options = {}) {
  ...
  try {
    ...
    if (config.google.applicationCredentials) {
      return await detectWithGoogle(imageBuffer, { minConfidence, maxResults });
    }

    // Use AWS Rekognition
    if (config.aws.accessKeyId) {
      return await detectWithAWS(imageBuffer, { minConfidence, maxResults });
    }

    // Use Hugging Face
    if (config.huggingface.apiKey) {
      return await detectWithHuggingFace(imageBuffer, { minConfidence, maxResults });
    }
...
async function detectWithGoogle(imageBuffer, options) {
  ...
  const objects = localizedObjects
    .filter(obj => obj.score >= options.minConfidence)
...
async function detectWithHuggingFace(imageBuffer, options) {
  ...
  const detections = response.data
    .filter(det => det.score >= options.minConfidence)
    .slice(0, options.maxResults)
    .map((det, index) => ({
      id: index,
      name: det.label,
