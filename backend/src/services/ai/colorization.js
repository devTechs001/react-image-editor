...
const colorize = async (imageBuffer, options = {}) => {
  try {
    ...
    const output = await replicate.run(
      'arielreplicate/deoldify_image:0da600fab0c45a66211339f1c16b71345d22f26ef5f7111e58f57907b4c3d7c5',
      {
        input: {
          input_image: dataUri,
...
const colorizeVideo = async (videoUrl, options = {}) => {
  try {
    const { renderFactor = 21 } = options;

    const output = await replicate.run(
      'arielreplicate/deoldify_video:0d8a6b1e6a5b5f9c8d2c3b4a5e6f7a8b9c0d1e2f',
      {
        input: {
...
module.exports = { colorize, colorizeVideo };