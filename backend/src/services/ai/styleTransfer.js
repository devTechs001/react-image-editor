...
const transfer = async (contentBuffer, styleBuffer, options = {}) => {
  try {
    const { styleStrength = 1.0 } = options;

    const contentBase64 = contentBuffer.toString('base64');
    const styleBase64 = styleBuffer.toString('base64');

    const output = await replicate.run(
      'tommoore515/neural_style_transfer:62bb8c6beeeaae6da2bf7c7e35ff05dc7ed17f19f9b0bc30bec2b05c5b299b37',
      {
        input: {
...
const applyPresetStyle = async (contentBuffer, styleName, options = {}) => {
  try {
    ...
    const output = await replicate.run(
      'jcjohnson/fast-neural-style:a]9a7fbd6f01e1c8fc77fc4a5c7f6e1c3f4f5e6d7c8b',
      {
        input: {
...
module.exports = { transfer, applyPresetStyle, getAvailableStyles };