...
const { uploadToStorage, getSignedDownloadUrl } = require('../storage/s3Storage');
...
const addTransition = async (clip1Path, clip2Path, transitionType = 'fade', duration = 1) => {
  ...
  const filterComplex = transitionType === 'fade'
    ? `[0:v][1:v]xfade=transition=fade:duration=${duration}:offset=auto[v];[0:a][1:a]acrossfade=d=${duration}[a]`
    : `[0:v][1:v]xfade=transition=${transitionType}:duration=${duration}:offset=auto[v];[0:a][1:a]acrossfade=d=${duration}[a]`;
...
const extractFrames = async (inputPath, options = {}) => {
  ...
  const { fps = 1, startTime = 0, duration = null } = options;
...
const convert = async (inputPath, outputFormat, options = {}) => {
  ...
  return new Promise((resolve, reject) => {
    ffmpeg(inputPath)
      .outputOptions(formatOptions[outputFormat] || [])
      .output(output)
      .on('end', () => resolve(output))
      .on('error', reject)
      .run();
