...
const { requireCredits, ownsResource } = require('../middleware/authorize');
...
const exportValidation = [
  ...
  body('format')
    ...
    .isIn(['png', 'jpg', 'jpeg', 'webp', 'gif', 'svg', 'pdf', 'mp4', 'webm', 'mov', 'mp3', 'wav'])
    .withMessage('Invalid export format'),
  body('quality').optional().isInt({ min: 1, max: 100 }),
  body('scale').optional().isFloat({ min: 0.1, max: 4 })
];

router.use(auth);

router.post('/',
  requireCredits('exports', 1),
