...
const { ownsResource } = require('../middleware/authorize');
...
const createProjectValidation = [
  ...
  body('type')
    ...
    .isIn(['image', 'video', 'audio']).withMessage('Invalid project type'),
  body('canvas.width')
    .notEmpty().withMessage('Canvas width is required')
    .isInt({ min: 1, max: 10000 }).withMessage('Width must be between 1 and 10000'),
  body('canvas.height')
    .notEmpty().withMessage('Canvas height is required')
    .isInt({ min: 1, max: 10000 }).withMessage('Height must be between 1 and 10000')
];
...
router.route('/')
  .get(cache(60), projectController.getProjects)
  .post(validate(createProjectValidation), clearCache('/projects*'), projectController.createProject);

router.route('/:id')
  .get(ownsResource('Project'), projectController.getProject)
  .put(ownsResource('Project'), validate(updateProjectValidation), projectController.updateProject)
  .delete(ownsResource('Project'), projectController.deleteProject);
