...
const layerSchema = new mongoose.Schema({
  ...
  data: mongoose.Schema.Types.Mixed, // Layer-specific data
...
projectSchema.index({ 'template.isTemplate': 1, 'template.category': 1 });
...
projectSchema.statics.findByUser = function(userId, options = {}) {
  const query = this.find({ user: userId, status: { $ne: 'deleted' } });
