...
userBehaviorSchema.index({ 'behaviors.type': 1, timestamp: -1 });
userBehaviorSchema.index({ 'analysis.successRate': -1 });
  ...
  const pipeline = [
    ...
    },
    {
      $group: {
        _id: null,
        totalSessions: { $sum: 1 },
        totalEvents: { $sum: '$analysis.totalEvents' },
        avgSessionDuration: { $avg: '$analysis.sessionDuration' },
        avgSuccessRate: { $avg: '$analysis.successRate' },
        avgErrorRate: { $avg: '$analysis.errorRate' },
        uniqueTools: { $addToSet: '$analysis.toolUsage' }
      }
    },
    {
      $project: {
        ...
        avgErrorRate: { $round: ['$avgErrorRate', 2] },
        uniqueToolsCount: { $size: '$uniqueTools' }
      }
    }
  ];
  ...
  const pipeline = [
    ...
    },
    {
      $group: {
        ...
        usageCount: { $sum: 1 },
        firstUsed: { $min: '$behaviors.timestamp' },
        lastUsed: { $max: '$behaviors.timestamp' }
      }
    },
    {
      $sort: { usageCount: -1 }
